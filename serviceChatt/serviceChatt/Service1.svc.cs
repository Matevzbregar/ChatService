...
namespace serviceChatt
{
    ...
    public class Service1 : IService1
    {
        ...
        public List<Message> Messages()
        {
            ...
            {
                con.Open();
                using (SqlCommand command = new SqlCommand("SELECT username, time, message FROM sporocila", con))
                using (SqlDataReader reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        ...
                        {
                            retVal.Add(new Message { Username = reader.GetString(0), Time = reader.GetString(1), Text = reader.GetString(2) });
                        }
                    }
                }
                con.Close();
        ...
        public List<Message> Messages(string id)
        {
            ...
            {
                con.Open();
                using (SqlCommand command = new SqlCommand("SELECT username, time, message FROM sporocila WHERE ID >= "+ id+"", con))
                using (SqlDataReader reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
